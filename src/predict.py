...
from .model_FasterRCNN import get_fasterrcnn_model
...
def predict(model, image_tensor, confidence_threshold, device):
    with torch.no_grad():
...
def main():
    ...
    TEST_DIR = "test"
    MAPPING_CSV = "data/ID_to_Image_Mapping.csv"
    ...
    with open(OUTPUT_CSV, "w", newline="") as f:
        ...
        for _, row in mapping_df.iterrows():
            ...
            if image_id in image_predictions_cache:
                cached_boxes, cached_scores, cached_labels = (
                    image_predictions_cache[image_id]
                )
            else:
                # Preprocess image
                ...
                boxes, scores, labels = predict(
                    model, image_tensor, CONFIDENCE_THRESHOLD, device
                )
            ...
            prediction_index = min(
                current_appearance - 1, len(cached_boxes) - 1
            )
            ...
            for b, s, l in zip(box, score, label, strict=True):
                # Convert label index to class name:
                #  (subtract 1 as index 0 is background)
                class_name = (
                    classes[l - 1]
                    if l > 0 and l <= len(classes)
                    else "Unknown"
                )
            ...
            print(
                ...
                f" (ID: {id_num}) - Appearance #{current_appearance}",
            )
